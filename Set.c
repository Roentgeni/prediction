...
void find_freq(Set *set) {
  ...
  printf("%s\n","predicting stability of features:");
    int n=set->num_fhc;
    n+=6;
    int freqs[n];
    HC **helices=set->helices;
    for(int i=0;i<n;i++){
      freqs[i]=(int)(helices[i]->freq);
    }
    clock_t time=clock();
    // for(int i=0;i<n;i++){printf("%d ",freqs[i]);}printf("\n");
    double fixed=repeatForNTimes(freqs,n,100,1);
    int result_f=fixed<0.09;
    printf("\tfixed : %f",fixed);
    if(result_f==1){
      printf("\t%s","stable");
    }else{
      printf("\t%s","unstable");
    ...
    for(int i=0;i<possible;i++){
      delta_i=delta[i];
      if(repeatForNTimes(freqs,n,10,delta_i)!=0){
	//var=delta_i;
	printf("%f",delta_i);
	break;
      }
      if(delta_i==delta[possible-1]){
	printf(">%f",delta_i);
      }
    }
    result_v=delta_i>1.9;
    if(result_v==1){
      printf("\t\t%s","stable");
    }else{
      printf("\t\t%s","unstable");
...
double repeatForNTimes(int* list,int length,int n,double m){
    int result[n];
    
    for(int i=0;i<n;i++){
      result[i]=calculatefeatures(list,length,m);
    }
    ...
    double sum1=0;
    for(int i=0;i<n;i++){
      sum1=sum1+pow((result[i]-average),2);
    }
    return sqrt(sum1/(float)n);
}

int calculatefeatures(int* list,int n,double coefficient){
    double freqs[n];
    double Hs[n];
    double hs[n];
    double value;
    ...
    double c1;
    double p;
    for(int i=0;i<n;i++){//generate a list of frequencies
        value= (double)list[i];
        //stdev=1.4658+0.0849344*value-2.34958*10**(-4)*value**2+3.2057*10**(-7)*value**3-1.70919*10**(-10)*value**4
        //better quadratic fit
        stdev=14.3348*sqrt(1-pow((value-500)/500,2));//eclipse
        //if(value>40.223){
	//  stdev=14.4298*sqrt(1-pow((value/500-1),2));
        //}else{
        //    stdev=0.140974*value;
        //}//picewise
	//printf("value: %f, stdev: %f\n",value,stdev);
	if(value>=1000.0){
	  freq=1000.0;
        }else{
            freq=random_normal(value,stdev*coefficient);
	}
	freqs[i]=freq;
    }
    //printf("%s\n","freqs: ");for(int i=0;i<n;i++){printf("%f ",freqs[i]);}printf("\n");
    //calculate entropy
    qsort(freqs,n,sizeof(freqs[0]),comp);
    c1=freqs[0]<1000?freqs[0]:1000;
    //printf("%s\n","sorted freqs: ");for(int i=0;i<n;i++){printf("%f ",freqs[i]);}printf("\n");    printf("c1: %f\n",c1);
    for(int i=0;i<n;i++){
        p=freqs[i]/c1;
        if(p>=1||p<=0){
            Hs[i]=0;
        }else{
            Hs[i]=-(p*log(p)+(1-p)*log(1-p));
        }
    }
    // printf("%s\n","Hs: ");for(int i=0;i<n;i++){printf("%f ",Hs[i]);}printf("\n");
    //cumulative sum
    double sum=0;
    for(int i=0;i<n;i++){
        sum=sum+Hs[i];
...
double random_normal(double mu,double sigma){
    return sqrt(-2*log(random_uniform()))*cos(2*M_PI*random_uniform())*sigma+mu;
}
